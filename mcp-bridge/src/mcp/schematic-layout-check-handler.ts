...
interface LayoutPrimitive { id: string; kind: PrimitiveKind; text?: string; designator?: string; x: number; y: number; rect: Rect; rotation: number; parentId?: string }
...
function lineRect(line: unknown): Rect | undefined {
	...
	if (!xs.length)
		return undefined;
	return { left: Math.min(...xs) - 4, right: Math.max(...xs) + 4, bottom: Math.min(...ys) - 4, top: Math.max(...ys) + 4 };
}
...
export async function handleSchematicLayoutCheckTask(payload: unknown): Promise<unknown> {
		...
		if (typeof componentApi.getAllPinsByPrimitiveId === 'function') {
			...
			if (Array.isArray(pins) && pins.length) {
				...
				for (let pinIndex = 0; pinIndex < pins.length; pinIndex += 1) {
					...
					primitives.push({ id: `${id}:pin:${text(getSyncState(pin, 'getState_PinNumber', pinIndex + 1))}`, kind: 'pin', x: px, y: py, rotation: num(getSyncState(pin, 'getState_Rotation', rotation)), rect: rectAround(px, py, 45, 45), parentId: id });
				}
			}
		}
		primitives.push({ id, kind: 'symbol', designator, x, y, rotation, rect: rectAround(x, y, width, height, rotation) });
	}
	...
	if (isPlainObjectRecord(attributeApi) && typeof attributeApi.getAll === 'function') {
		...
		for (let index = 0; index < rawAttributes.length; index += 1) {
			...
			primitives.push({ id, kind: 'attribute', text: value, x, y, rotation, parentId, rect: rectAround(x, y, Math.max(fontSize * 0.6, value.length * fontSize * 0.6), fontSize * 1.3, rotation) });
		}
	}
	...
	if (isPlainObjectRecord(wireApi) && typeof wireApi.getAll === 'function') {
		...
		if (Array.isArray(wires)) {
			wires.slice(0, MAX_PRIMITIVES).forEach((wire, index) => {
				const rect = lineRect(getSyncState(wire, 'getState_Line', null)); if (rect)
					primitives.push({ id: primitiveId(wire, `wire-${index}`), kind: 'wire', x: (rect.left + rect.right) / 2, y: (rect.bottom + rect.top) / 2, rotation: 0, rect });
			});
		}
	}
	...
	for (let i = 0; i < primitives.length; i += 1) {
		for (let j = i + 1; j < primitives.length; j += 1) {
			const a = primitives[i]; const b = primitives[j]; if (a.parentId && a.parentId === b.id || b.parentId && b.parentId === a.id)
				continue;
			...
			collisions.push({ primitiveIds: [a.id, b.id], collisionType: type, severity: severity(a.kind, b.kind, area), area, primitives: [{ id: a.id, kind: a.kind, x: a.x, y: a.y, rect: a.rect, text: a.text, designator: a.designator }, { id: b.id, kind: b.kind, x: b.x, y: b.y, rect: b.rect, text: b.text, designator: b.designator }] });
		}
	}

	const denseRegions: Record<string, unknown>[] = []; const cells = new Map<string, LayoutPrimitive[]>(); const cellSize = 1000;
	for (const primitive of primitives) { const key = `${Math.floor(primitive.x / cellSize)},${Math.floor(primitive.y / cellSize)}`; const list = cells.get(key) ?? []; list.push(primitive); cells.set(key, list); }
	...
	const outOfBounds = pageBounds ? primitives.filter(item => item.rect.left < pageBounds.left || item.rect.right > pageBounds.right || item.rect.bottom < pageBounds.bottom || item.rect.top > pageBounds.top).map(item => ({ primitiveId: item.id, kind: item.kind, rect: item.rect })) : [];
